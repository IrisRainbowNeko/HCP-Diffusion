...
class FluxShiftTimeSampler(TimeSampler):
    def __init__(self, t_sampler: TimeSampler = None, base_shift: float = 0.5, max_shift: float = 1.15):
        self.t_sampler = t_sampler
    ...
    def sample(self, min_t=0.0, max_t=1.0, shape=(1,), H=0, W=0) -> torch.Tensor:
        mu = self.get_lin_function(H*W, y1=self.base_shift, y2=self.max_shift)
        t = self.t_sampler.sample(min_t, max_t, shape)
